...
public class NaturalSpawnerMixin {
	...
	private static boolean overrideNaturalSpawnCondition(ServerLevel level, Mob entity, double f) {
		var result = EntityEvents.LIVING_CHECK_SPAWN.invoker().canSpawn(entity, level, entity.xOld, entity.yOld, entity.zOld, EntitySpawnReason.NATURAL, null);
	...
	private static boolean overrideChunkGenSpawnCondition(Mob mob, LevelAccessor level, EntitySpawnReason type) {
		var result = EntityEvents.LIVING_CHECK_SPAWN.invoker().canSpawn(mob, level, mob.xOld, mob.yOld, mob.zOld, EntitySpawnReason.CHUNK_GENERATION, null);
		if (result != InteractionResult.PASS) {
			return result.consumesAction();
		}
